...
export type QuizMeta = {
  slug: string;          // e.g. bg/5-kids OR sb/1/1-adult
  scripture: "bg" | "sb";
  audience: Audience;
  difficulty: string;
  title: string;
...
export function getQuizBySlug(slug: string): QuizData {
...
function parseBgSlug(slug: string) {
  // bg/5-adult or bg/5-kids
  const m = slug.match(/^bg\/(\d+)-(adult|kids)$/);
  if (!m) return null;
  return { chapter: Number(m[1]), audience: m[2] as Audience };
}

function parseSbSlug(slug: string) {
  // sb/1/1-adult or sb/1/1-kids
  const m = slug.match(/^sb\/(\d+)\/(\d+)-(adult|kids)$/);
  if (!m) return null;
  return { canto: Number(m[1]), chapter: Number(m[2]), audience: m[3] as Audience };
}

export function listQuizMetas(): QuizMeta[] {
...
export function getLatestQuizzes(limit = 10): QuizMeta[] {
...
export function getBgAvailability() {
  ...
  const map = new Map<string, QuizMeta>(); // key: `${chapter}-${audience}`
...
export function getSbAvailability() {
  ...
  const map = new Map<string, QuizMeta>(); // key: `${canto}/${chapter}-${audience}`
  metas.forEach((m) => map.set(`${m.canto}/${m.sbChapter}-${m.audience}`, m));
  return map;
}

export function listSbCantos(): number[] {
...
export function listSbChaptersInCanto(canto: number): number[] {
